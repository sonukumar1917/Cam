...
async def admin_panel(message: types.Message):
    ...
    # Use text with emojis – no invalid 'icon_custom_emoji_id' parameter
    kb.row(types.InlineKeyboardButton(text=f"{Emojis.STATS} Get Stats", callback_data="admin_stats"))
    kb.row(types.InlineKeyboardButton(text=f"{Emojis.BROADCAST} Broadcast", callback_data="admin_broadcast"))
    kb.row(types.InlineKeyboardButton(text=f"{Emojis.TICKET} Give Credit", callback_data="admin_give_credit"))
...
async def admin_stats_cb(call: types.CallbackQuery):
    await call.answer()  # ✅ Must acknowledge
...
async def admin_broadcast_cb(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.from_user.id not in ADMIN_IDS:
        ...
        return
    await call.message.edit_text(
        f"{Emojis.BROADCAST} <b>Enter the message to broadcast:</b>",
        parse_mode="HTML"
    )
...
async def broadcast_process(message: types.Message, state: FSMContext, bot: Bot):
    ...
    # Confirm that users is a list of IDs (not tuples)
    if users and isinstance(users[0], tuple):
        users = [u[0] for u in users]

    await message.answer(
        f"{Emojis.ROCKET} Starting broadcast to {len(users)} users...",
        parse_mode="HTML"
    )
    ...
    await message.answer(
        f"{Emojis.CHECK} Broadcast completed! Sent to {count} users.",
        parse_mode="HTML"
    )
...
async def give_credit_cmd(message: types.Message):
    ...
    if len(args) < 3:
        await message.answer(
            f"{Emojis.CROSS} Usage: /give_credit [user_id] [amount]",
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        await message.answer(
            f"{Emojis.CHECK} Successfully added {amount} credits to user <code>{user_id}</code>",
            parse_mode="HTML"
        )
    except ValueError:
        await message.answer(
            f"{Emojis.CROSS} Invalid User ID or Amount. Must be numbers.",
            parse_mode="HTML"
        )
    except Exception as e:
        await message.answer(
            f"{Emojis.CROSS} Error: <code>{str(e)}</code>",
            parse_mode="HTML"
        )
...
async def broadcast_cmd(message: types.Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    await message.answer(
        f"{Emojis.BROADCAST} <b>Enter the message to broadcast to all users:</b>",
        parse_mode="HTML"
    )
    await state.set_state(AdminForm.waiting_for_broadcast)
...
async def set_tutorial_cmd(message: types.Message):
    ...
    if not video_id:
        await message.reply(
            f"{Emojis.CROSS} Please attach a video or reply to a video with /set_tutorial",
            parse_mode="HTML"
        )
        return
    await database.set_setting("tutorial_video", video_id)
    await message.reply(
        f"{Emojis.CHECK} Tutorial video updated successfully!",
        parse_mode="HTML"
    )
...
async def set_bots_cmd(message: types.Message):
    ...
    if not text:
        await message.reply(
            f"{Emojis.CROSS} Usage: /set_bots <text> (or reply to a text message)",
            parse_mode="HTML"
        )
        return
    await database.set_setting("bots_list", text)
    await message.reply(
        f"{Emojis.CHECK} Bots list updated successfully!",
        parse_mode="HTML"
